...
export default function ProductCard({ product }) {
  ...
  const { imageSrc, isLoading, hasError } = useImageWithRetry(
    product.thumbnail
  );
    ...
    <article className="group bg-white rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden card-hover animate-fade-in">
        ...
        {isLoading ? (
          <div className="w-full h-48 sm:h-56 lg:h-64 bg-gray-200 animate-pulse flex items-center justify-center">
            <div className="flex flex-col items-center">
              ...
              <span className="text-sm text-gray-400 mt-2">Cargando...</span>
            </div>
          </div>
        ) : hasError ? (
      ...
      <div className="p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
          <h4
            className="text-sm sm:text-base font-semibold text-secondary-900 line-clamp-2 group-hover:text-primary-600 transition-colors duration-200 cursor-pointer"
            onClick={handleClick}
            onKeyDown={handleKeyDown}
            tabIndex="0"
            role="button"
        ...
        <button
          ...
          className={`w-full mt-4 px-4 py-2 font-medium rounded-lg transition-colors duration-200 focus-ring btn-hover ${
            product.stock === 0
