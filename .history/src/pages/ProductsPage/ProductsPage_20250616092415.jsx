...
export default function ProductsPage() {
  ...
  // Estado separado para el slider y los inputs
  const [sliderValues, setSliderValues] = useState({ min: 0, max: 2000 });
  const [inputValues, setInputValues] = useState({ min: 0, max: 2000 });
  ...
  // Filtrar y ordenar productos usando los valores del slider
  useEffect(() => {
    ...
    // Filtrar por rango de precio usando sliderValues
    filtered = filtered.filter(
      (product) =>
  ...
  }, [products, selectedCategory, selectedOrder, sliderValues, searchTerm]);

  // Sincronizar inputs con slider cuando el slider no está activo
  useEffect(() => {
    if (!isSliderActive) {
      setInputValues(sliderValues);
    }
  }, [sliderValues, isSliderActive]);
  ...
  // Handler para el slider - solo actualiza sliderValues
  const handleSliderChange = useCallback((values) => {
    setIsSliderActive(true);
    setSliderValues({ min: values.min, max: values.max });

    // Desactivar el flag después de un breve delay
    setTimeout(() => setIsSliderActive(false), 100);
  }, []);

  // Handlers para los inputs - actualizan tanto inputValues como sliderValues
  const handleMinPriceInput = (value) => {
    const numValue = Math.max(0, Math.min(value, sliderValues.max - 1));
    const newValues = { min: numValue, max: sliderValues.max };
    setInputValues(newValues);
    setSliderValues(newValues);
  };

  const handleMaxPriceInput = (value) => {
    const numValue = Math.min(2000, Math.max(value, sliderValues.min + 1));
    const newValues = { min: sliderValues.min, max: numValue };
    setInputValues(newValues);
    setSliderValues(newValues);
  };
  ...
  const handleReset = () => {
    ...
    setSearchTerm('');
    const resetValues = { min: 0, max: 2000 };
  ...
  const activeFiltersCount = [
    ...
    sliderValues.min > 0 || sliderValues.max < 2000,
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 sticky top-6">
              ...
              <div className="mb-8">
                ...
                <div className="space-y-4">
                  ...
                  {/* Slider de rango - solo onInput para evitar conflictos */}
